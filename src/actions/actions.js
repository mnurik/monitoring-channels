...
const editChannel = (payload) => ({ type: actionTypes.EDIT_CHANNEL, payload });
...
export const deleteChannel = (id) => (dispatch) => {
    return api.deleteChannel(id)
        .then(() => { dispatch(destroyChannel(id)); })
        .catch(error => { toastr.error(error); });
};
...
export const saveChannel = (data) => (dispatch) => {
    return api.saveChannel(data)
        .then(response => {
            if (data.id) dispatch(editChannel(response));
            else dispatch(addChannel(response));
